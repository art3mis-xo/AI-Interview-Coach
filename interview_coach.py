...
class InterviewCoach:
    ...
    def __init__(
        ...
        if job_description_path:
            ...
            self.question_generator = create_question_generator(retriever)
    ...
    def get_default_topics_for_position(self) -> List[str]:
        """Return role-appropriate default interview topics."""
        position = self.position.lower()

        ai_keywords = [
            "ai engineer", "ai engg", "ai engineering", "machine learning",
            "ml engineer", "llm", "generative ai", "genai", "data scientist"
        ]

        if any(keyword in position for keyword in ai_keywords):
            return [
                "LLM fundamentals",
                "prompt engineering",
                "RAG architecture",
    ...
    def start_interview(self, session_id: str, topics: List[str] = None) -> str:
        ...
        if topics is None:
            topics = self.get_default_topics_for_position()
